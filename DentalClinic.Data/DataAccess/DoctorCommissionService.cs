...
public class DoctorCommissionService
{
    ...
    public void RecordCommissionForPayment(int sessionId, int paymentId, decimal paidAmount)
    {
        ...
        _financialRepo.AddDoctorCommissionExpense(commissionAmount, sessionId, paymentId, doctorId, doctorName, percent);
    }

    // يُعاد حساب عمولة يوم واحد بالكامل لطبيب معيّن من الصفر - يُستدعى بعد أي تعديل أو حذف على دفعة
    // تخصّه (راجع PaymentRepository.UpdatePaymentAmount/DeletePayment)، بدل محاولة تحديث تراكمي قد
    // ينحرف مع الوقت. يحذف صف العمولة القديم لذلك اليوم بالذات ويعيد إنشاءه من جديد إن كان لا يزال
    // مستحقاً، بناءً على مجموع كل الدفعات الفعلية المتبقية لهذا الطبيب في ذلك اليوم بعد التعديل.
    public void RecalculateCommissionForDoctorDate(int doctorUserId, DateTime date)
    {
        var dateOnly = date.Date;

        const string deleteSql = @"
            DELETE FROM ClinicExpenses
            WHERE Category = @Cat AND SourceDoctorUserID = @DoctorUserID AND CAST(ExpenseDate AS DATE) = @Date";
        _db.ExecuteNonQuery(deleteSql,
            new SqlParameter("@Cat", FinancialRepository.DoctorCommissionCategory),
            new SqlParameter("@DoctorUserID", doctorUserId),
            new SqlParameter("@Date", dateOnly));

        var primaryId = GetPrimaryDoctorUserId();
        if (primaryId.HasValue && primaryId.Value == doctorUserId) return; // الطبيب الرئيسي لا عمولة عليه أبداً

        const string sumSql = @"
            SELECT ISNULL(SUM(p.Amount), 0) AS TotalPaid
            FROM Payments p
            INNER JOIN MedicalSessions s ON s.SessionID = p.SessionID
            WHERE s.DoctorID = @DoctorUserID AND CAST(p.PaymentDate AS DATE) = @Date";

        var totalPaid = Convert.ToDecimal(_db.ExecuteScalar(sumSql,
            new SqlParameter("@DoctorUserID", doctorUserId),
            new SqlParameter("@Date", dateOnly)));

        if (totalPaid <= 0) return;

        var percent = GetEffectiveCommissionPercent(doctorUserId);
        if (percent <= 0) return;

        var commissionAmount = Math.Round(totalPaid * percent / 100m, 2);
        if (commissionAmount <= 0) return;

        var doctorName = _userRepo.GetAllDoctors().FirstOrDefault(d => d.UserID == doctorUserId)?.FullName ?? "Unknown";
        ...
        const string insertSql = @"
            ...
            VALUES (@Amount, @Desc, @Cat, @Date, 1, NULL, NULL, @DoctorUserID)";

        _db.ExecuteNonQuery(insertSql,
            new SqlParameter("@Amount", commissionAmount),
