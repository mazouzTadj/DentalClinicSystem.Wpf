...
// شاشة إدارة المستخدمين - يجب ألا تُفتح إلا من المدير العام (IsAdmin = true)
public partial class UserManagementWindow : Window
{
    ...
    public UserManagementWindow(UserAccount currentUser)
    {
        ...
        // لن تُنفَّذ أي عملية إدارية ما لم يكن المستخدم الحالي مديراً عاماً فعلاً.
        if (!currentUser.IsAdmin)
        {
            throw new UnauthorizedAccessException("Only a Super Admin can open User Management.");
        }
    ...
    {
        var window = new UserEditWindow(existingUser: null) { Owner = this };
    ...
    private void EditSelected()
    {
        ...
        var window = new UserEditWindow(existingUser: selected.User) { Owner = this };
        if (window.ShowDialog() == true)
        {
            LoadUsers();
        }
